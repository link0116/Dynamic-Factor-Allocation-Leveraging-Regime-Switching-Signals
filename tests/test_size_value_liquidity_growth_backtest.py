...
from experiments.size_value_liquidity_growth_backtest import (
    EXPERIMENT_FACTORS,
    run_size_value_liquidity_growth_experiment,
)
from strategy.multifactor_backtest import MultiFactorBacktestConfig


class SizeValueLiquidityGrowthBacktestTests(unittest.TestCase):
    @patch("experiments.size_value_liquidity_growth_backtest.run_multifactor_backtest")
    def test_experiment_only_changes_factors_dates_top_n_and_output(self, run_backtest) -> None:
        ...
        with TemporaryDirectory() as output_dir:
            result = run_size_value_liquidity_growth_experiment(
        ...
        self.assertEqual(config.factor_names, ("size", "value", "liquidity", "growth"))
    ...
    def test_existing_results_are_not_overwritten(self, run_backtest) -> None:
        with TemporaryDirectory() as output_dir:
            ...
            with self.assertRaisesRegex(FileExistsError, "避免覆盖原结果"):
                run_size_value_liquidity_growth_experiment(output_dir=output_dir)
