"""仅使用 Size、Value、Liquidity 因子的选股对比实验。"""
...
EXPERIMENT_FACTORS = ("size", "value", "liquidity")


def run_size_value_liquidity_experiment(
    start_date: str | None = None,
    end_date: str | None = None,
    top_n: int = 100,
    output_dir: str = "outputs/size_value_liquidity_backtest",
) -> dict[str, object]:
    """保持主回测其余默认条件不变，仅用三个指定因子计算 Alpha。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Size、Value、Liquidity 三因子选股对比实验")
    parser.add_argument("--start-date", default=None)
    parser.add_argument("--end-date", default=None)
    parser.add_argument("--top-n", type=int, default=100)
    parser.add_argument("--output-dir", default="outputs/size_value_liquidity_backtest")
    args = parser.parse_args()
    result = run_size_value_liquidity_experiment(
